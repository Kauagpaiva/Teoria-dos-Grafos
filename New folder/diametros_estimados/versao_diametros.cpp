...
public:
    ...
    int getGraphDiameter() {
        ...
        for (int i = 1; i <= V; ++i) {
            if (!visited[i]) {
                int diameter = getComponentDiameter(i, visited);
...
//
    ...
    // Função de busca em profundidade (DFS) iterativa usando pilha
    void dfs(int start) {
        vector<bool> visited(V + 1, false);
        ...
        // Pilha para realizar a DFS iterativa
        stack<int> dfsStack;
        dfsStack.push(start);
        level[start] = 0;
        ...
        while (!dfsStack.empty()) {
            ...
            if (!visited[u]) {
                ...
                // Escreve o vértice atual, pai e nível no arquivo
...
private:
    ...
    // BFS para encontrar a maior distância a partir de um nó inicial
    pair<int, int> bfs(int start) {
        ...
        while (!q.empty()) {
            ...
            for (int v : adjList[u]) {
                if (dist[v] == -1) {  // Ainda não foi visitado
                    dist[v] = dist[u] + 1;
                    q.push(v);
                    farthestNode = v;
                }
    ...
    int getComponentDiameter(int start, vector<bool> &visited) {
        ...
        pair<int, int> farthestFromStart = bfs(start);

        // Marcar todos os vértices da componente como visitados
        queue<int> q;
        q.push(start);
        visited[start] = true;
        ...
        pair<int, int> farthestFromFarthest = bfs(farthestFromStart.first);
        return farthestFromFarthest.second;  // O maior valor de distância é o diâmetro
    }
...
// }
 ...
 int main() {
     ...
     g.loadFromFile("graph6.txt"); //lendo o arquivo

     int diametro = g.getGraphDiameter();

     std::ofstream outputfile("diametro_estimado.txt");
     outputfile << "diametro estimado: " << diametro << endl;
     
     outputfile.close();
    
    //g.connectedComponents();

    // int distancia1 = g.bfsDistance(10,20) ;
    // int distancia2 = g.bfsDistance(10,30) ;
    // int distancia3 = g.bfsDistance(20,30) ;
    //  
    // std::ofstream outputFile("distancias.txt");
    // outputFile << "distancia entre 10 e 20: " << distancia1 <<endl;
    // outputFile << "distancia entre 10 e 30: " << distancia2 <<endl;
    // outputFile << "distancia entre 20 e 30: " << distancia3 <<endl;
    // outputFile.close();


    // srand(static_cast<unsigned int>(time(0)));

    // int min = 1, max = g.getV();
...
//
    ...
    //    outputFile << "tempo medio: " << media << " microsegundos" << std::endl;
    //    outputFile.close();  // Close the file to save the results
    //    std::cout << "Results written to timing_results.txt" << std::endl;
    //} else {
